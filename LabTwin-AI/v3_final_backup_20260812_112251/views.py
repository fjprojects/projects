...
3. ONE clear viva question that checks the exact misconception.
...
def evaluate_code(request):
    ...
    try:
        ...
        task = Task(
            description=f"""
You are a FAIR programming-lab viva evaluator.

Your job is semantic assessment, not keyword matching.
...
SUGGESTED RUBRIC CONCEPTS:
{json.dumps(expected_concepts, indent=2)}

FIRST validate the suggested rubric concepts.

RUBRIC VALIDATION RULES:
- Keep only concepts that are factually correct and relevant to the viva question.
- Drop or repair ambiguous, duplicated, misleading, or factually incorrect criteria.
- Do not penalize the student for a detail that the viva question did not reasonably ask.
- Do not require exact wording, textbook phrases, or exact syntax unless the viva question explicitly asks for syntax and syntax is central to the topic.
- Applied examples in the student's answer count as evidence of understanding.
- Use at most 4 independent criteria.

For each validated criterion assign ONLY one rating:

2 = clearly demonstrated and conceptually correct, including a correct paraphrase or applied example.
1 = partially or implicitly demonstrated; the core idea is present but an important detail is missing.
0 = absent or conceptually wrong.

IMPORTANT:
- A mostly correct answer with one omitted detail must NOT receive 0%.
- "contradictions" is ONLY for explicit statements that directly conflict with a central concept.
- Missing information is NOT a contradiction.
- Do NOT assign the final percentage or mastery status. Django will do that deterministically.
...
{{
    "criteria": [
        {{
            "criterion": "Validated concept",
            "rating": 2,
            "evidence": "Short evidence from the student's answer"
        }}
    ],
    "contradictions": [],
    "reason": "Short fair evidence-based feedback"
}}
""",
            expected_output="Validated semantic viva rubric",
        ...
        if not validated_criteria:
            raise ValueError(
                "AI returned an empty viva rubric. Please retry evaluation."
            )

        # Deterministic semantic scoring:
        # 2 = 100 points
        # 1 = 60 points (meaningful partial understanding)
        # 0 = 0 points
        rating_points = {
            0: 0,
            1: 60,
        ...
        understanding = round(
            sum(
                ...
                for item in validated_criteria
            )
        ...
        # Only an explicit central contradiction can strongly cap
        # conceptual understanding. Omissions/partial answers do not.
        if contradictions:
            understanding = min(
                understanding,
                35
        ...
        return JsonResponse({
            ...
            # The frontend replaces this with persisted dashboard
